...
# Import langchain prompts, chains, and memory
...
# Import Contextual_compression as retrievers
...
st.title("🤖 RAG Chatbot For Multiple Data Types")
...
def langchain_document_loader():
    """
    Create documnet loaders for PDF, TXT and CSV files.
    Files are loaded from TMP_DIR as documents.
    ...
    with st.spinner("Creating vectorstore..."):
        ...
        if len(error_messages) == 1:
            ...
            try:
                ...
                delte_temp_files()
                print("Temp File Delete Success")

                # 2. Upload selected documents to temp directory
                if st.session_state.uploaded_file_list is not None:
                    print("Uploaded Files List: ", st.session_state.uploaded_file_list)
                    for uploaded_file in st.session_state.uploaded_file_list:
                        ...
                        try:
                            ...
                            print(f"Creating temp file at: {temp_file_path}")
                    ...
                    print("Document Success Uploaded")

                    # 3. Load documents with Langchain loaders
                    documents = langchain_document_loader()
                    # print(f"Loaded Documents: {documents}")

                    print("Document Success Loaded")

                    # 4. Split documents to chunks
                    chunks = split_documents_to_chunks(documents)
                    print(f"Document Chunks Length: {len(chunks)}")
                    # print(f"Document Chunks Sample: {chunks[:1]}")  # Print first chunk sample for inspection

                    print("Document Success Splitted")
                    
                    # 5. Embeddings
                    embeddings = select_embeddings_model()
                    print(f"Selected Embeddings Model: {embeddings}")

                    sentences = ["I like pets.",
                                "Pets bring joy to our lives.",
                                "Langchain is a framework for developing applications powered by LLMs."]
                    # 1. Calculate embedding vectors
                    embedding_vectors = [embeddings.embed_query(sentence) for sentence in sentences]
                    ...
                    print("Document Success Embedded")

                    # Manually create embeddings for chunks and check their output
                    # embedding_results = [embeddings.embed_documents(chunk.page_content) for chunk in chunks]
                    # print("Embedding Results Sample: ", embedding_results[:1])
                    ...
                    print(f"Persist Directory Path: {persist_directory}")

                    print("Vector Store Created")

                    try:
                        # print(f"Number of Chunks: {len(chunks)}")
                        # print(f"Sample Chunk: {chunks[0] if len(chunks) > 0 else 'No Chunks'}")
                        st.session_state.vector_store = Chroma.from_documents(
                            documents=chunks,
                            embedding=embeddings,
                            persist_directory=persist_directory
                        )
                        print(f"Vector Store Created: {st.session_state.vector_store}")
                        st.info(
                        ...
                        print("Retriever Success Created")

                        # 8. Create memory and ConversationalRetrievalChain
                        print("Creating Memory and Conversational Retrieval Chain...")
                        (
                            st.session_state.chain,
                            st.session_state.memory,
                        ) = create_ConversationalRetrievalChain(
                        ...
                        print("Memory and Conversational Retrieval Chain Success Created")
                        ...
                        print(f"Error in Vector Store Creation or Retriever Setup: {e}")
                        st.error(e)

            except Exception as error:
                print(f"General Error in Process: {error}")
